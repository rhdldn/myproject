...
def getLolRank(userId, userType):
    ...
    for userInfo in userList:
        ...
        soloRankInfo = commonUtil.curs.fetchone()

        #자유 랭크 정보 조회
        commonUtil.curs.execute(sql, ("RANKED_FLEX_SR", userId))        
        freeRankInfo = commonUtil.curs.fetchone()
        ...
        sql += "   ORDER BY A.GAME_DATE DESC "
        sql += "   LIMIT 8) A "

        commonUtil.curs.execute(sql, userId)        
        lastGameList = commonUtil.curs.fetchall()            

        userJson = json.dumps(userInfo, default=commonUtil.json_default)
        soloRankJson = json.dumps(soloRankInfo, default=commonUtil.json_default)
        freeRankJson = json.dumps(freeRankInfo, default=commonUtil.json_default)
        lastGameJson = json.dumps(lastGameList, default=commonUtil.json_default)

        retJson["userMap"] = userInfo
        retJson["soloRankMap"] = soloRankInfo
        retJson["freeRankMap"] = freeRankInfo
        retJson["lastGameList"] = lastGameList    
